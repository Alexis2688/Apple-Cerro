<x-layout>
    @section('title', 'Facturación por Días - Compras')

    @section('content')
        <div class="bg-white rounded-lg shadow-md p-6">
            <h2 class="text-2xl font-semibold text-gray-800 mb-4">Facturación por Día - Compras</h2>

            <!-- Formulario para filtrar por fecha -->
            <form action="{{ route('compras.facturacion') }}" method="GET" class="mb-6 flex items-center gap-4">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Total Comprado</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($facturacionPorDia as $factura)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form action="{{ route('compras.eliminarPorFecha', ['fecha' => $factura->fecha]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center py-4 text-gray-500">No se encontraron compras para esta fecha.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
