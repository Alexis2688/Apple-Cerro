<x-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-gray-500 mt-1">Gestión de compras de productos de Insumo</p>
                </div>
                <div class="flex flex-col sm:flex-row gap-3">
                    <a href="{{ route('compras-insumos.create') }}"
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 mb-6 border border-gray-100">
            <form action="{{ route('compras-insumos.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
        ...
        <form id="delete-form" action="{{ route('compras-insumos.destroy-multiple') }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="font-medium text-gray-900">{{ $compra->producto }}</span>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-gray-500">${{ number_format($compra->precio_unitario, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap font-bold text-gray-900">${{ number_format($compra->total, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap flex space-x-2">
                                <a href="{{ route('compras-insumos.edit', $compra->id) }}"
                                   class="text-blue-600 hover:text-blue-900 transition-colors flex items-center"
                                   title="Editar">
                                    <i class="fas fa-edit mr-1"></i>
                                </a>
                                <form action="{{ route('compras-c.destroy', $compra->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
        ...
        <div class="mt-6 flex flex-col md:flex-row justify-between items-center">
            <div class="bg-white rounded-lg shadow-sm p-4 mb-4 md:mb-0">
                <h3 class="text-lg font-semibold text-gray-800">
                    Total de Compras: <span class="text-blue-600">${{ number_format($totalCompras, 2) }}</span>
                </h3>
            </div>
    ...
    <script>
        ...
        document.getElementById('eliminar-seleccionados').addEventListener('click', async function() {
            ...
            if (confirm(`¿Estás seguro de eliminar ${ids.length} compras seleccionadas?`)) {
                try {
                    const response = await fetch("{{ route('compras-insumos.destroy-multiple') }}", {
                        method: 'POST',
                        headers: {
